...
- 已完成 HUD 候选按钮选择扩张目标。
- 已完成 gameplay command 服务承接 UI / 快捷键调用。
- 本轮做敌方定时进攻。
- 后续再做正式点击地图或更完整 UI。
...
## 当前正式任务：MVP-04.1 敌方最小压力 AI

目标：

```text
敌人定时派兵进攻玩家目标，让游戏具备最低限度压力；本轮不做 enemy capture。
```

实现方向：

```diff
+ 新增 EnemyAttackCommandService 或等价敌方进攻命令服务
+ 新增 EnemyPressureController 定时调用敌方进攻
+ HUD 显示最近敌方行动或下一次进攻状态
+ E debug 快捷键立即触发一次敌方进攻
- 不做 enemy capture
- 不做完整行为树 / 第三方 AI 框架
- 不做资源/升级/区域奖励
- 不做正式 UI 美术
